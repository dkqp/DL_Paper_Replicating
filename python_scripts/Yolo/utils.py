    ...
    def loss_fn(output, target):
        ...
        for i in range(len(target)):
            ...
            for j, (r, c, m) in enumerate(conf_coord):
                target_box[r, c, m] = torch.tensor(1)
...
def prediction_evaluation(iou_th=0.5):
    '''
    ...
    def pred_eval(preds, target):
        # for checking if there are classes not exist
        num_target_by_classes = Counter([int(t[0]) for t in target])

        # make output dictionary with keys as classes
        preds_checked = {}

        # i: iterate for each class to check if the prediction is true or false
        for i, pred in enumerate(preds):
            if num_target_by_classes[i] == 0:
                continue

            # sort predictions as confidence order
            pred_sorted = sorted(pred, key=lambda x: x[0], reverse=True)
            # check which target boxes are not available
            duplication_check = set([])

            pred_checked = [False] * len(pred_sorted)
            for j, p in enumerate(pred_sorted):
                # for a prediction box, there is up to one target box with highest iou value, which is possibly matched
                iou_max = 0
                iou_max_index = None
                # check all target boxes
                for k, t in enumerate(target):
                    # except the boxes which are not available, because they are already matched by prior predictions
                    if k in duplication_check:
                        continue
                    iou_temp = calculate_iou(p[1:], t[1:])
                    # check the class and minimum iou threshold condition
                    if t[0] == i and iou_temp > iou_th:
                        if iou_temp > iou_max:
                            iou_max = iou_temp
                            iou_max_index = k
                # a target box which has the highest iou value among all available boxes is selected, and the prediction is should be True
                if iou_max_index is not None:
                    pred_checked[j] = True
                    duplication_check.add(iou_max_index)

            # predictions with (confidence, True/False) would be returned
...
def mean_average_precision(pred_checked, num_target):
    '''
    Calculate mAP from the prediction boxes

    pred_checked: hashmap with key of classes and value of list of T/F with confidence
    num_target: Counter for counting targets for each class
    returns the average of APs of each class
    '''

    # AP values for each class
    AP = []
    for key in pred_checked.keys():
        # for calculate recall
        num_target_of_class = num_target[key]
        if num_target_of_class == 0:
            continue

        # sorted by confidence to calculate precision and recall
        all_pred_sorted = sorted(pred_checked[key], key=lambda x: x[0], reverse=True)
        # precision values for 11 sections of recall values between 0 to 1
        precision_recall_11 = []

        # calculate precision and recall
        precision = [0, 0]
        recall = [0, num_target_of_class]
        ...
        for _, p in all_pred_sorted:
            ...
            # fill the points of the prediction-recall curve
        ...
        # fill zeros for the rest points after checking all predictions
...
def score_fn_Yolo(preds_checker):
    '''
    Calculate mAP score for all test images

    preds_checker: prediction evaluation function

    returns a function which calculates the score
    ...
    def score(preds, target):
        ...
        for i in range(len(preds)):
            # predictions of each image are evaluated
            preds_checked = preds_checker(preds[i], target[i])

            # predictions are accumulated by predicted class
        ...
        # the numbers of all target boxes of the entire images are calculated
        num_target = Counter([])
        for i in range(len(target)):
            num_target += Counter([int(t[0].item()) for t in target[i]])

        # calculate mAP and return the value
